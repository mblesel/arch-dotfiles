...
-- Pick a markdown heading and insert a link to it at the cursor
function MdInsertHeadingLink()
    local bufnr = vim.api.nvim_get_current_buf()

    -- Parse markdown headings with tree-sitter
    local parser = vim.treesitter.get_parser(bufnr, "markdown")
    local root = parser:parse()[1]:root()
    local query = vim.treesitter.query.parse("markdown", [[
        (atx_heading
            (_) @marker
            heading_content: (inline) @content)
    ]])
    ...
    for id, node in query:iter_captures(root, bufnr, 0, -1) do
        ...
        if name == "marker" then
            current.level = #vim.trim(vim.treesitter.get_node_text(node, bufnr))
        elseif name == "content" then
            current.text = vim.trim(vim.treesitter.get_node_text(node, bufnr))
            table.insert(items, {
                text = string.rep("#", current.level) .. " " .. current.text,
                heading = current,
            })
            current = {}
        end
    ...
    if vim.tbl_isempty(items) then
        vim.notify("No markdown headings found", vim.log.levels.WARN)
        return
    end
    ...
    require("snacks").picker({
        ...
        format = function(item)
            return {
                { string.rep("  ", item.heading.level - 1) },
                { item.heading.text },
            }
        end,
        confirm = function(picker, item)
            ...
            local slug = item.heading.text:lower()
                :gsub("[`*_~]", "")
                :gsub("[^%w%s%-]", "")
                :gsub("^%s+", ""):gsub("%s+$", "")
                :gsub("%s+", "-")

            local link = string.format("[%s](#%s)", item.heading.text, slug)
